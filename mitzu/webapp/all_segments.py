...
class AllSegmentsContainer(html.Div):
    ...
    def fix(
        cls,
        complex_segs: List[bc.Component],
        metric: M.Metric,
        ...
        for i, seg_child in enumerate(complex_segs):
            if i >= limit:
                break
    ...
    def create_callbacks(cls, webapp: WA.MitzuWebApp):
        ...
        def input_changed(
            ...
            metric_type: str,
            pathname: str,
            all_seg_dicts: List[Dict],
            mc_dicts: List[Dict],
        ):
            webapp.load_dataset_model(pathname)
            complex_segs: List[bc.Component] = [
                deserialize_component(child) for child in all_seg_dicts
            ]
            mc_children: List[bc.Component] = [
                deserialize_component(child) for child in mc_dicts
            ]
            ...
            metric = create_metric(
                complex_segs=complex_segs,
                mc_children=mc_children,
                discovered_datasource=dm,
                metric_type=metric_type,
            )

            res_children = cls.fix(complex_segs, metric)
